...
class PortalFirestoreDB:
    def __init__(self, db, portal_id='fcm'):
        """Initialize database with FCM-specific collections"""
        self.db = db
        self.portal_id = 'fcm'  # Always use fcm
        self.urls_ref = self.db.collection('fcm_monitored_urls')  # FCM specific collection
        self.portal_ref = self.urls_ref.document('fcm')
        
        # Initialize encryption
        self._init_encryption()
        
        # Initialize FCM subcollections
        self.scraper_state_ref = self.portal_ref.collection('fcm_scraper_states')
        self.scraper_history_ref = self.portal_ref.collection('fcm_scraper_history')
        self.error_ref = self.portal_ref.collection('fcm_errors')
        self.invoice_ref = self.portal_ref.collection('fcm_invoices')
        self.settings_ref = self.portal_ref.collection('fcm_settings')
        self.scheduler_settings_ref = self.settings_ref.document('scheduler')
        self.credentials_ref = self.portal_ref.collection('fcm_credentials')
    ...
    def _ensure_collections_exist(self):
        """Ensure all required FCM collections exist with initial documents"""
        collections_data = {
            'fcm_scraper_states': {'initialized': True, 'type': 'fcm_state_collection'},
            'fcm_scraper_history': {'initialized': True, 'type': 'fcm_history_collection'},
            'fcm_errors': {'initialized': True, 'type': 'fcm_error_collection'},
            'fcm_invoices': {'initialized': True, 'type': 'fcm_invoice_collection'},
            'fcm_settings': {'initialized': True, 'type': 'fcm_settings_collection'},
            'fcm_members': {'initialized': True, 'type': 'fcm_members_collection'},
            'fcm_files': {'initialized': True, 'type': 'fcm_files_collection'}
        }
        ...
        for coll_name, init_data in collections_data.items():
            ...
            if not self._check_collection_exists(coll_ref):
                ...
                logger.info(f"Initialized FCM collection: {coll_name}")
