...
class AkasaScraper:
    ...
    def emit_status(self, stage, status, message, timing=None, error=None):
        ...
        if self.socketio:
            try:
                ...
                # Update emit events to be Akasa-specific
                ...
                self.socketio.emit('akasa_scraper_stage', data)
                self.socketio.emit('akasa_scraper_general_status', {
                    'status': status,
                    'message': f"{stage.title()}: {message}",
                    'timestamp': datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                })
                ...
                # Add debug event
                self.socketio.emit('akasa_scraper_debug', {
                    'stage': stage,
                    'status': status,
                    'message': message,
                    'timing': timing,
                ...
                logging.debug(f"Emitted Akasa status: {data}")
            except Exception as e:
                socket_logger.log_error(stage, f"Failed to emit Akasa status: {str(e)}")
                logger.error(f"Akasa socket emission error: {e}")
    ...
    def process_request(self, pnr, lastName, traveller_name=None):
        """Process request without retries"""
        ...
        try:
            lastName = lastName.replace(" ", "%20")  # Add URL encoding for spaces
            ...
            self.timing_data['request_time'] = round(request_time, 3)
            
            return response

        except Exception as e:
            error_msg = f"Request failed: {str(e)}"
            self.emit_status(self.current_stage, 'error', error_msg, error=e)
            raise Exception(error_msg)

    def process_response(self, response, pnr, lastName, traveller_name):
        """Simplified response processing matching raw scraper"""
        ...
        try:
            ...
            # No need to look for specific invoice content - just check response code
            # The raw scraper considers 200 status code as success
            self.emit_status(self.current_stage, 'success', 'Response saved successfully')
...
def run_scraper(data, db_ops, socketio=None):
    """Main scraper entry point with improved name handling"""
